...
* 单一职责原则：应该有且仅有一个原因引起类的变更。要求：接口一定要做到单一职责，类的设计尽量做到只有一个原因引起变化。
* 里氏替换原则：所有引用基类的地方必须能透明地使用其子类的对象。要求：尽量去除子类的个性，只要父类能出现的地方子类就可以出现，而且替换为子类也不会产生任何错误或异常，使用者可能根本就不需要知道是父类还是子类。但反过来未必可行。
* 依赖倒置原则：高层模块不应该依赖低层模块，两者都应该依赖其抽象；抽象不应该依赖细节；细节应该依赖抽象。要求：通过抽象（接口或者实现类）使各个类或者模块的实现解耦。
* 接口隔离原则：类之间的依赖应该建立在最小的接口上。要求：接口的粒度应该小、单一职责、高内聚，一个接口只服务于一个子模块或者业务逻辑。
* 迪米特原则（最少知识原则）：一个对象应该对其它（需要耦合或者调用的）对象有最少的了解。要求：类之间解耦——一个类只和需要的类产生耦合、适当控制成员的访问权限、如果不产生负面影响不增加类间耦合一个方法如果放在本类中。
* 开闭原则：一个软件实体例如类、模块、函数等英国对扩展开放、对修改关闭。要求：应该通过扩展来实现变化，而不是修改已有代码。要求：预知变化，通过抽象来约束、封装变化，实现对扩展开放；尽量通过配置文件控制模块的行为；制定项目章程。
