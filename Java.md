...
* 类型擦除的特点：
    ...
    * 类型擦除确保了程序能和JDK5之前的版本开发二进制类库进行兼容
    * 类型擦除之后，无法在运行时访问到类型参数（反射出来的结果为原始类型），因为编译器已经把泛型类型转换成了原始类型。因此：通过反射的方式可以实现关于泛型的非法操作
    
#### 3. 泛型的通配符

* 限定通配符：对类型进行了限制。泛型类型必须用限定内的类型来进行初始化，否则会导致编译错误

    * ```java<? extends T>```：它通过确保类型**必须是T的子类**来设定类型的**上界**
    * ```java<? super T>```：它通过确保类型**必须是T的父类**来设定类型的**下界**
